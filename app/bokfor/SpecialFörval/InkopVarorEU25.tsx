...
export default function InkopVarorEU25(props: Props) {
  const {
    mode,
    belopp,
    setBelopp,
  ...
  if (mode === "steg2") {
    const handleLocalSubmit = () => {
      ...
      const extrafaltObj = {
        ...
        "4010": { label: "Inköp material och varor", debet: val, kredit: 0 },
        "4515": { label: "Inköp av varor från annat EU-land, 25 %", debet: val, kredit: 0 },
        "4598": { label: "Justering, omvänd moms", debet: 0, kredit: val },
      };
    ...
    return (
      <div className="p-6 bg-cyan-950 text-white border border-cyan-800 rounded-2xl shadow-lg">
        <h1 className="mb-6 text-3xl text-center text-white">Steg 2: Inköp varor inom EU 25%</h1>
        ...
        <div className="flex flex-col-reverse justify-between h-auto max-w-5xl px-4 mx-auto md:flex-row">
          <div className="w-full mb-10 md:w-[40%] md:mb-0 bg-slate-900 border border-gray-700 rounded-xl p-6 text-white">
            ...
            <Falt label="Totalbelopp" type="number" value={total} onChange={setTotal} />
            <Falt
              label="Kommentar"
              type="textarea"
              value={kommentar ?? ""}
  ...
  if (mode === "steg3") {
    ...
    const rows = [
      ...
      { konto: "4010 Inköp material och varor", debet: rad["4010"]?.debet ?? 0, kredit: 0 },
      {
        konto: "4515 Inköp av varor från annat EU-land, 25 %",
        debet: rad["4515"]?.debet ?? 0,
        kredit: 0,
      },
