...
const formatSEK = (val: number): string =>
  val.toLocaleString("sv-SE", { minimumFractionDigits: 2 });

export default function InkopTjanstEU(props: Props) {
  const {
    mode,
    belopp,
    setBelopp,
  ...
  if (mode === "steg2") {
    ...
    return (
      <div className="p-6 bg-cyan-950 text-white border border-cyan-800 rounded-2xl shadow-lg">
        <h1 className="mb-6 text-3xl text-center text-white">Steg 2: Inköp tjänster inom EU 25%</h1>
        ...
        <div className="flex flex-col-reverse justify-between h-auto max-w-5xl px-4 mx-auto md:flex-row">
          <div className="w-full mb-10 md:w-[40%] md:mb-0 bg-slate-900 border border-gray-700 rounded-xl p-6 text-white">
            ...
            <Falt label="Totalt inköpsbelopp" type="number" value={total} onChange={setTotal} />
            <Falt
              label="Kommentar"
              type="textarea"
              value={kommentar ?? ""}
    ...
    return (
      <main className="min-h-screen text-white bg-slate-950 px-4">
        <div className="max-w-5xl mx-auto bg-cyan-950 border border-cyan-800 rounded-2xl shadow-lg p-10">
          ...
          <p className="text-center font-bold text-xl mb-1">Inköp tjänster inom EU 25%</p>
