...
export default function InkopTjanstEU({
  ...
  if (mode === "steg2") {
    ...
    return (
      <section className="bg-cyan-950 text-white">
        <h1 className="mb-6 text-3xl text-center">Steg 2: Inköp tjänster inom EU 25%</h1>
  ...
  if (mode === "steg3") {
    const rows = Object.entries(extrafält).map(([konto, data]) => ({
      konto: `${konto} ${data.label}`,
      debet: data.debet,
      kredit: data.kredit,
    }));
