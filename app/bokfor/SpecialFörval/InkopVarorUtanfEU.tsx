...
export default function InkopVarorUtanfEU(props: Props) {
  const {
    mode,
    belopp,
    setBelopp,
  ...
  if (mode === "steg2") {
    const handleLocalSubmit = () => {
      ...
      const extrafaltObj = {
        ...
        "4500": { label: "Inköp utanför Sverige", debet: val, kredit: 0 },
    ...
    return (
      <div className="bg-cyan-950 text-white">
        <h1 className="mb-6 text-3xl text-center text-white">Steg 2: Inköp varor utanför EU</h1>
  ...
  if (mode === "steg3") {
    ...
    const rows = [
      ...
      },
      {
        ...
        debet: rad["4010"]?.debet ?? 0,
        kredit: 0,
      },
      {
        konto: "4500 Inköp utanför Sverige",
        debet: rad["4500"]?.debet ?? 0,
        kredit: 0,
      },
      {
        konto: "4598 Justering, omvänd moms",
        debet: 0,
        kredit: rad["4598"]?.kredit ?? 0,
      },
    ];
    ...
    return (
      <main className="min-h-screen text-white bg-slate-950 px-4">
        <div className="max-w-5xl mx-auto bg-cyan-950 border border-cyan-800 rounded-2xl shadow-lg p-10">
          ...
          <p className="text-center font-bold text-xl mb-1">Inköp varor utanför EU</p>
